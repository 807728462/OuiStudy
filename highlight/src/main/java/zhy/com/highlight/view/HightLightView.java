...
//            mPaint.setMaskFilter(new BlurMaskFilter(DEFAULT_WIDTH_BLUR, BlurMaskFilter.Blur.SOLID));
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        if (mViewPosInfo.interceptHighlightOver) {
            return true;
        }
        ...
        if (mViewPosInfo.interceptHighlightOver) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    return checkoutLigh(event);
            }
        }
        return super.onTouchEvent(event);
    }

    private boolean checkoutLigh(MotionEvent event) {
        if (null != mViewPosInfo) {
            float x = event.getX();
            float y = event.getY();
            int l = (int) mViewPosInfo.rectF.left;
            int t = (int) mViewPosInfo.rectF.top;
            int r = (int) mViewPosInfo.rectF.right;
            int b = (int) mViewPosInfo.rectF.bottom;
            if (x > l && x < r && y > t && y < b) {
                Log.d("HighlightActivity", "高亮区域--内部-" + x + "-" + y + "-" + l + "-" + t + "-" + r + "-" + b);
                return super.onTouchEvent(event);
            } else {
                Log.d("HighlightActivity", "高亮区域--外部-" + x + "-" + y + "-" + l + "-" + t + "-" + r + "-" + b);
            }
        }
        return true;
    }
